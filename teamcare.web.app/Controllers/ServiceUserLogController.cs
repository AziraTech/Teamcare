...
namespace teamcare.web.app.Controllers
{
	...
	public class ServiceUserLogController : BaseController
	{
		...
		public async Task<IActionResult> IsApprove(Guid id, bool status)
		{
			try
			{
				var servicelog = await _serviceUserLogService.UpdateLogByParam(1, id, status, null, (Guid)base.UserId, 0);
		...
		public async Task<IActionResult> IsInVisible(Guid id, bool status)
		{
			try
			{
				var servicelog = await _serviceUserLogService.UpdateLogByParam(2, id, status, null, (Guid)base.UserId, 0);
		...
		[HttpPost]
		public async Task<IActionResult> UpdateLog(Guid id, String logtext, ServiceUserLogCategory logCategory)
		{
			try
			{
				var servicelog = await _serviceUserLogService.UpdateLogByParam(3, id, false, logtext, (Guid)base.UserId, logCategory);
		...
		[HttpPost]
		public async Task<IActionResult> SortFilterOptionList(Guid? filterByserviceuser, bool IsApprove, string daterange, ServiceUserLogCategory logCategory)
		{
			try
			{
				var listOfLog = await _serviceUserLogService.ListAllSortedFiltered(filterByserviceuser, IsApprove, daterange, logCategory);
